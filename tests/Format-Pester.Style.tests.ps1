...
Describe 'Style rules' -Tags "Style"{
    ...
    $files = @(
        ...
        Get-ChildItem (Join-Path $RelativePathToModuleRoot 'Public') -Include *.ps1, *.psm1, *.psd1 -Recurse
        Get-ChildItem (Join-Path $RelativePathToModuleRoot '..\Tests') -Include *.ps1, *.psm1 -Recurse
        Get-ChildItem (Join-Path $RelativePathToModuleRoot '..\demo') -Include *.ps1, *.psm1 -Recurse
    )
